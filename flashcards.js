...
var charArray = ["a", "i", "u", "e", "o", "ka", "ki", "ku", "ke", "ko", "sa", "shi", "su", "se", "so", "ta", "chi", "tsu", "te", "to", "na", "ni", "nu", "ne", "no", "ha", "hi", "fu", "he", "ho", "ma", "mi", "mu", "me", "mo", "ya", "yu", "yo", "ra", "ri", "ru", "re", "ro", "wa", "wo", "n"]
var x = Math.floor(Math.random() * 46)
var randomChar = charArray[x]
console.log(characters[randomChar])
var hiragana = true

function repopulate() {
    if(hiragana) {
        randChar = characters[randomChar].hiragana
    } else {
        randChar = characters[randomChar].katakana
    }
    var hSpace =  "<div class='card " + characters[randomChar].row +"' value='" + characters[randomChar].file + "'>" + randChar + "</div>"
        $(".container").append(hSpace)
} repopulate()
...
//Switches to hiragana
$(document).on("click", ".h-switch", function(){
    hiragana = true
    $(".container").html("")
    repopulate()
})
//Creates new random card based on object
$(document).on("click", ".newCard", function(){
    x = Math.floor(Math.random() * 46)
    randomChar = charArray[x]
    console.log(characters[randomChar])
    $(".container").html("")
    repopulate()
})
